...
-- -------------------------------------------------------
INSERT INTO sys_menu (menu_name, title, icon, path, paths, menu_type, action, permission, parent_id, no_cache, breadcrumb, component, sort, visible, is_frame, create_by, update_by, created_at, updated_at, deleted_at)
SELECT 'GameManage', '游戏管理', 'ep:trophy', '/game', '/0/', 'M', '无', '', 0, false, '', 'Layout', 90, '0', '0', 1, 1, NOW(), NOW(), NULL
WHERE NOT EXISTS (
    SELECT 1 FROM sys_menu WHERE path = '/game' AND menu_type = 'M' AND deleted_at IS NULL
);

-- -------------------------------------------------------
-- 2. 插入六个子菜单（C 类型，parent_id 通过子查询获取）
-- -------------------------------------------------------

-- 游戏管理
INSERT INTO sys_menu (menu_name, title, icon, path, paths, menu_type, action, permission, parent_id, no_cache, breadcrumb, component, sort, visible, is_frame, create_by, update_by, created_at, updated_at, deleted_at)
SELECT 'GameList', '游戏列表', 'ep:list', '/game/list', CONCAT('/0/', (SELECT menu_id FROM sys_menu WHERE path = '/game' AND menu_type = 'M' AND deleted_at IS NULL LIMIT 1), '/'), 'C', '无', 'game:game:list', (SELECT menu_id FROM sys_menu WHERE path = '/game' AND menu_type = 'M' AND deleted_at IS NULL LIMIT 1), false, '', 'game/index', 0, '0', '0', 1, 1, NOW(), NOW(), NULL
WHERE NOT EXISTS (
    SELECT 1 FROM sys_menu WHERE path = '/game/list' AND menu_type = 'C' AND deleted_at IS NULL
);
