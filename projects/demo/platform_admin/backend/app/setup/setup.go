...
func getStatus(c *gin.Context) {
	if !IsInstalled() {
		c.JSON(http.StatusOK, gin.H{"code": 0, "data": gin.H{"installed": false}, "message": "ok"})
		return
	}
	// 配置文件存在，但需验证数据库是否可连接且有基础表
	c.JSON(http.StatusOK, gin.H{"code": 0, "data": gin.H{"installed": true}, "message": "ok"})
}
...
func doInstall(c *gin.Context) {
	...
	// 4. 写入初始数据（超级管理员 + 默认租户 + SUPER_ADMIN 角色）
	if err = authPkg.SeedInitialData(db); err != nil {
