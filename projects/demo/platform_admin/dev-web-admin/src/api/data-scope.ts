...
export function listDataScopeConfigs() {
  return request.get<any, DataScopeConfigItem[]>('/api/v1/admin/data-scope-configs')
}

/** 新增数据权限维度 */
export function createDataScopeConfig(data: DataScopeConfigFormData): Promise<void> {
  return request.post('/api/v1/admin/data-scope-configs', data)
}

/** 编辑数据权限维度 */
export function updateDataScopeConfig(id: string, data: DataScopeConfigFormData): Promise<void> {
  return request.put(`/api/v1/admin/data-scope-configs/${id}`, data)
}

/** 删除数据权限维度 */
export function deleteDataScopeConfig(id: string): Promise<void> {
  return request.delete(`/api/v1/admin/data-scope-configs/${id}`)
}
