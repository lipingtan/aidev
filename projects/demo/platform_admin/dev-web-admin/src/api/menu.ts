...
export function fetchUserMenu(): Promise<MenuItem[]> {
  return request.get('/api/v1/common/user-menu?platform=admin')
}

/** 获取当前用户角色菜单（侧边栏用） */
export function getUserMenuTree(): Promise<MenuTreeItem[]> {
  // 新 RBAC 接口，失败时返回空数组不阻塞
  return request.get<any, MenuTreeItem[]>('/api/v1/common/user-menu?platform=admin').catch((): MenuTreeItem[] => [])
}
