...
export function listApplications() {
  return request.get<any, ApplicationItem[]>('/api/v1/applications')
}

/** 新增应用 */
export function createApplication(data: ApplicationFormData): Promise<void> {
  return request.post('/api/v1/applications', data)
}

/** 编辑应用 */
export function updateApplication(id: string, data: ApplicationFormData): Promise<void> {
  return request.put(`/api/v1/applications/${id}`, data)
}

/** 删除应用 */
export function deleteApplication(id: string): Promise<void> {
  return request.delete(`/api/v1/applications/${id}`)
}
...
export function updateTenantApps(tenantId: string, appCodes: string[]): Promise<void> {
  return request.put(`/api/v1/tenants/${tenantId}/apps`, { app_codes: appCodes })
}
