...
export async function listAllPlugins() {
  return request.get('/api/v1/admin/plugins')
}

/** 启用插件 */
export function enablePlugin(name: string) {
  return request.put(`/api/v1/admin/plugins/${name}/start`)
}

/** 禁用插件 */
export function disablePlugin(name: string) {
  return request.put(`/api/v1/admin/plugins/${name}/stop`)
}
...
export function installPluginByUrl(data: { name: string; url: string }) {
  return request.post('/api/v1/admin/plugins/install', data)
}
...
export function installPluginByFile(name: string, file: File) {
  ...
  return request.post('/api/v1/admin/plugins/install', formData, {
