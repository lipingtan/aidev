...
import { getPlayerList, getPlayerById, banPlayer } from "@/api/game";

defineOptions({ name: "PlayerManage" });
...
async function loadData() {
  loading.value = true;
  try {
    const res: any = await getPlayerList(query);
    list.value = res.data || [];
...
async function openDetail(row: any) {
  const res: any = await getPlayerById(row.id);
...
async function onBanSubmit() {
  ...
  try {
    await banPlayer(currentId.value, { status: 2, banReason: banForm.banReason });
...
async function onUnban(row: any) {
  await banPlayer(row.id, { status: 1, banReason: "" });
