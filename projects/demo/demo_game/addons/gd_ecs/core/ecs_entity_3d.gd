class_name EcsEntity3D extends CharacterBody3D
## ECS Entity 3D（extends CharacterBody3D）
##
## 3D 物理角色 Entity，适用于需要移动、碰撞的 3D 游戏对象。
## 内置 ECS Component 管理能力 + CharacterBody3D 物理能力。
##
## 适用场景：玩家角色、NPC、敌人、可推动的物体。
