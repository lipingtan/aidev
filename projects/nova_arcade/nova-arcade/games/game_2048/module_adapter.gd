extends GameModule
## 2048 协议适配器（CR-6 T2；tetra 模式）
## - boot(ctx)：注入 SAVE_PATH = ctx.save_dir + "save.cfg" → src
## - src on_game_over(stats) → quit_requested({score, playtime, achievements:[], extra:{max_tile}})
...
func quit_to_shell() -> void:
	quit_requested.emit({
		...
		"achievements": [],
		"extra": {"max_tile": int(_last_stats.get("max_tile", 0))},
	})
...
## 游戏结束：记录统计（Shell 统一结算卡，adapter 不展示）
