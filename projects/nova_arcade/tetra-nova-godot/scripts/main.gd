...
const GameClass := preload("res://scripts/game.gd")
...
func _ready() -> void:
    ...
    bg = preload("res://scripts/star_bg.gd").new()
    add_child(bg)

    board_view = preload("res://scripts/board_view.gd").new()
    shaker.add_child(board_view)

    fx = preload("res://scripts/fx_layer.gd").new()
    shaker.add_child(fx)
    fx.board_view = board_view

    au = preload("res://scripts/audio_manager.gd").new()
    add_child(au)
