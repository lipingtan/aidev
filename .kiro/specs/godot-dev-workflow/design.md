...
- **Part A: 工作流规范体系**（Req 1-10）— 以 Markdown 规范文件 + steering 配置形式落地
- **Part B: 技术架构体系**（Req 11-12）— 以 Godot addon 插件代码形式落地
...
│   ├── object-pool.md              # 对象池模式
│   └── behavior-tree.md            # 行为树模式
├── templates/
│   ├── rpg/
...
Phase 0: 项目初始化
    ├── 创意轨道: 确认游戏类型/题材/平台
    └── 技术轨道: 创建目录骨架/Godot工程/技术选型
         │
Phase 1: 核心设计
    ├── 创意轨道: GDD → 世界观 → 核心玩法设计
    └── 技术轨道: 技术架构 → ECS框架配置 → 模块接口定义
         │ ← 同步点: 世界观确认 → 触发架构设计
         │
Phase 2: 原型验证
    ├── 创意轨道: 角色设计 → 关卡草案
    └── 技术轨道: 核心玩法原型 → 手感验证 → AI行为验证
         │ ← 同步点: 角色确认 → 触发角色系统
         │
Phase 3: 核心系统实现
    ├── 创意轨道: 剧情大纲 → 支线设计 → 对话脚本
    └── 技术轨道: 角色系统 → 战斗系统 → 叙事系统 → UI → 存档
         │ ← 同步点: 关卡确认 → 触发场景搭建
         │
Phase 4: 内容填充
    ├── 创意轨道: 关卡细化 → 敌人设计 → 道具设计
    └── 技术轨道: 关卡实现 → 敌人配置 → 道具数据 → 音效集成
         │
Phase 5: 打磨优化
    ├── 创意轨道: 平衡性评审 → 体验优化建议
    └── 技术轨道: 性能优化 → Bug修复 → 平衡调整 → 打包
```
...
| 设计对象 | 生成时机 | 粒度 | 理由 |
|----------|----------|------|------|
| **系统框架** | 该类设计全部完成后 | 一次性生成整个系统的接口+骨架 | 确保接口一致性 |
| **角色个体** | 逐个设计完即生成 | 每个角色的数据(.tres)+专属逻辑 | 可独立验证，互不阻塞 |
| **关卡** | 单个关卡设计完整后 | 该关卡的全部配置一次性生成 | 关卡内元素耦合度高 |
| **道具/技能** | 批量设计后批量生成 | 同类数据一起生成 | 保持格式一致性 |
| **3D 资产** | 逐个导入 | 每个资产单独适配 | 依赖外部工具，周期长 |
| **2D 资产** | 批量生成 | 同风格一起生成 | AI 批量处理效率更高 |

#### 角色设计→代码流程

```
1. 设计所有角色的"共性"（属性结构、Component 定义）
   → 生成：角色系统框架代码（ECS Component 基类、属性系统）
   → 验证：框架代码可编译、Component 可在 Inspector 中编辑

2. 逐个角色设计"个性"（具体数值、技能、行为）
...
```
1. 设计关卡的"结构"（区域划分、流程节点、胜利条件）
   → 生成：关卡场景骨架(.tscn) + 区域触发器 + 流程控制脚本

2. 设计关卡的"内容"（敌人配置、道具放置、对话触发）
...
- 第一层（骨架）完成 → 触发架构设计 POST-CHECK
- 第二层（实现）每完成一个单元 → 触发单元级 POST-CHECK（语法+基础行为）
- 第三层（集成）完成 → 触发集成测试 POST-CHECK（完整流程可跑通）
...
│   └── ecs_query.gd            # Query 构建器
├── debug/
│   ├── ecs_debugger.gd         # 调试面板
│   ├── entity_inspector.gd     # Entity 查看器
...
| 决策点 | 选择 | 理由 |
|--------|------|------|
| Entity 基类 | extends Node | 保留场景树集成、编辑器可视化 |
| Component 基类 | extends Resource | 支持 Inspector 编辑、序列化、.tres 存储 |
| System 基类 | extends RefCounted | 轻量、无节点开销、纯逻辑 |
| Query 机制 | 基于 Component 名称数组匹配 | 简单直观、支持动态注册 |
| 调度方式 | World 在 _process/_physics_process 中遍历 | 与 Godot 帧循环一致 |
| 动态注册 | Dictionary 注册表 + 信号通知 | 支持 DLC 运行时注入 |
| 状态机集成 | 状态机作为 Entity 子节点，通过读写 Component 通信 | 行为决策与数据处理分层 |
...
| 层 | 负责什么 | 示例 |
|---|---|---|
| **状态机** | 单个角色的行为逻辑、状态转换条件、动画控制 | "血量<30%时从 Attack 切换到 Flee" |
| **ECS System** | 跨角色的批量计算、全局规则 | "所有有 PoisonBuff 的 Entity 每秒扣 5 血" |
| **ECS Component** | 纯数据存储 | HP、攻击力、Buff 列表、当前状态枚举 |

##### Entity 节点结构

```
Entity (EcsEntity extends Node)
├── StateMachine (Node)              ← Godot 原生节点，管理状态转换
│   ├── IdleState
│   ├── RunState
│   ├── AttackState
│   ├── HitState
│   └── DieState
├── Component: CharacterStats        ← ECS 纯数据
├── Component: Velocity              ← ECS 纯数据
├── Component: CombatState           ← ECS 纯数据（状态机写入，System 读取）
└── Component: BuffList              ← ECS 纯数据
```
...
##### ECS System 示例（读取 Component 批量处理）
...
2. 新建 ShadowComponent（暗影能量值等数据）→ 注册到 ECS
3. 可选：新建 ShadowDrainSystem（每秒消耗暗影能量）→ 注册到 ECS

无需修改任何现有代码：
- 状态机通过添加新状态节点扩展
- ECS 通过注册新 Component + System 扩展
...
EcsWorld._ready()
    ...
    └── 扫描场景树中的 EcsEntity 节点并注册
    │
    ▼
每帧循环
    │
    ├── EcsWorld._process(delta)
    │   └── 遍历 phase="process" 的 System
    │       └── 对每个 System: query → 获取匹配 Entity → system.process(entities, delta)
    │
    └── EcsWorld._physics_process(delta)
        └── 遍历 phase="physics_process" 的 System
            └── 同上
    │
    ▼
Entity 生命周期
    ├── 进入场景树 → _enter_tree() → EcsWorld.register_entity(self)
    └── 离开场景树 → _exit_tree() → EcsWorld.unregister_entity(self)
```
...
DlcManager.load_dlc(dlc_id)
    │
    ├── 1. 读取 manifest.json → 解析为 DlcManifest
    │
    ├── 2. 版本兼容性检查
    │   └── IF 不兼容 → emit dlc_load_failed → return
    │
    ├── 3. 依赖检查
    │   └── IF 依赖未加载 → 递归加载依赖
    │
...
│   ├── dlc_package.gd          # DLC 包封装
│   ├── dlc_manifest.gd         # 清单解析
│   ├── dlc_validator.gd        # 版本/依赖/冲突验证
│   └── dlc_loader.gd           # 文件加载（目录/PCK）
├── integration/
│   ├── ecs_integration.gd      # 与 ECS 框架的集成层
│   └── asset_integration.gd    # 与资产注册表的集成层
└── examples/
    ├── example_dlc/             # 示例 DLC 包
    │   ├── manifest.json
    │   ├── components/
    │   └── systems/
    └── dlc_test_scene.tscn
...
### B3. RPG 数值系统设计（属性/装备/技能/成长/容器）

#### 系统总览

```
┌─────────────────────────────────────────────────────────────┐
│ RPG 数值系统 — Component/System/Resource 分层                 │
├─────────────────────────────────────────────────────────────┤
│                                                              │
...
StatsCalculationSystem.process()
    ...
    │
    ├── 2. 收集所有 StatModifier 来源
    │   ├── Equipment.slots → 每件装备的 stat_modifiers
    │   ├── BuffList.active_buffs → 每个 Buff 的 modifiers
    │   └── SkillSet.learned_skills → 被动技能的 passive_modifiers
    │
    ├── 3. 按类型分组计算
    │   ├── FLAT_ADD: 所有固定值求和
    │   ├── PERCENT_ADD: 所有百分比求和后乘算
    │   └── PERCENT_MULT: 逐个独立乘算
    │
    ├── 4. 最终公式
...
@export var is_dirty: bool = true   # 标记是否需要重算
```
...
| 容器类型 | 容量 | 过滤器 | 只读 | 持久化 | 特殊规则 |
|----------|------|--------|------|--------|----------|
| 背包 | 40（可扩展） | 无 | 否 | 存档 | 负重限制（可选） |
| 装备栏 | 7（固定槽位） | 按槽位类型 | 否 | 存档 | 装备/卸下触发属性重算 |
| 储物柜 | 100 | 无 | 否 | 存档 | 多角色共享 |
| 商店 | 无限 | 无 | 是 | 不存 | 有价格、可刷新库存 |
| 宝箱 | 1~10 | 无 | 是 | 场景状态 | 开启后标记已开 |
| 掉落物 | 1~5 | 无 | 否 | 不存 | 有过期时间 |
| 交易窗口 | 临时 | 无 | 否 | 不存 | 双方确认后执行 |
...
## 容器间物品转移请求（由 UI 层生成，由 ContainerSystem 消费）
...
├── Component: Interactable (interaction_range=2.0, prompt="打开储物柜")
├── MeshInstance3D
└── CollisionShape3D
...
# 4. 存档时序列化 STORAGE 类型容器的 slots 数据
```
...
└─────────────┘                      └──────────────────────┘
       ↕ 物品转移                            ↑
┌─────────────┐                      ┌──────────────────────┐
│ 储物柜       │                      │ SkillCooldownSystem   │
│ (Storage)   │                      │ (技能CD递减)          │
└─────────────┘                      └──────────────────────┘
       ↕ 拾取                                ↑
┌─────────────┐                      ┌──────────────────────┐
│ 掉落物/宝箱  │                      │ BuffTickSystem        │
│ (Loot)      │                      │ (Buff持续/过期)       │
└─────────────┘                      └──────────────────────┘
       ↕ 买卖
┌─────────────┐
...
| 扩展内容 | 实现方式 |
|----------|----------|
| 新属性维度（如"暗影抗性"） | 新 Component 或扩展 BaseStats + 修改计算公式 |
| 新装备槽位 | EquipmentComponent.slots 是 Dictionary，直接加新 key |
| 新技能类型 | 新 SkillData 资源 + 可选的新 SkillEffect 类型 |
| 新成长路线 | 新 GrowthProfile 资源文件 |
| 新物品类型 | 新 ItemType 枚举值 + 对应处理逻辑 |
| 新容器类型 | 新 ContainerType + 对应 UI 面板 |
| 新物品效果 | 新 UseEffect 脚本注册到效果处理器 |
...
攻击发起
    ...
    │   └── Hitbox(Area3D) 与 Hurtbox(Area3D) 重叠 → 生成 DamageEvent
    │
    ├── 2. 伤害计算
    │   ├── 基础伤害 = 攻击者.FinalStats.attack * 技能倍率
    │   ├── 防御减伤 = 基础伤害 * (1 - 防御者.defense / (防御者.defense + 常数))
    │   ├── 元素克制 = 减伤后 * 元素倍率表[攻击元素][防御元素]
    │   ├── 暴击判定 = rand() < 攻击者.crit_rate → 伤害 * crit_damage
    │   └── 格挡判定 = 防御者.is_blocking → 伤害 * block_reduction
    │
    ├── 3. 伤害应用
    │   └── 防御者.CharacterStats.hp -= final_damage
    │
    └── 4. 后处理
        ├── 生成伤害数字 UI
        ├── 触发受击状态（状态机 → HitState）
...
```
角色 Entity
├── Hurtbox (Area3D, 始终激活)     ← 受击区域
├── Hitbox_Weapon (Area3D, 默认关闭) ← 武器攻击区域
│   └── 由状态机在攻击动画关键帧激活/关闭
└── Hitbox_Skill (Area3D, 动态生成)  ← 技能特效区域
...
@export var tick_interval: float = 1.0       # 周期触发间隔（0=不触发）
@export var max_stacks: int = 1
@export var stack_mode: StackMode = StackMode.REFRESH
@export var stat_modifiers: Array[StatModifier] = []  # 属性修饰
@export var tick_effects: Array[SkillEffect] = []     # 周期效果
@export var on_apply_effects: Array[SkillEffect] = [] # 施加时效果
@export var on_remove_effects: Array[SkillEffect] = [] # 移除时效果
@export var immunity_tags: Array[StringName] = []     # 免疫此 Buff 的标签
...
```
敌人 Entity
├── StateMachine (高层状态：巡逻/战斗/逃跑/死亡)
│   └── CombatState 内部使用行为树做决策
├── BehaviorTree (Node)          ← 战斗决策
│   ├── Selector: 选择行为
│   │   ├── Sequence: 远程攻击（距离>5 且 有弹药）
│   │   ├── Sequence: 近战攻击（距离<2）
│   │   ├── Sequence: 接近目标（距离>2）
│   │   └── Action: 待机
│   └── 每帧 tick → 输出决策 → 写入 Component
├── Component: AiState           ← AI 决策数据
├── Component: AggroTable        ← 仇恨表
├── Component: PatrolRoute       ← 巡逻路径
└── Component: CombatState       ← 共用战斗状态
```
...
@export var phase_thresholds: Array[float] = [0.7, 0.4, 0.15]  # HP 百分比
...
@export var result_skill: StringName          # 触发的技能
...
class ComboInput:
    ...
    var max_interval: float = 0.5 # 与上一个输入的最大间隔
...
| 数据类别 | 来源 | 存档方式 |
|----------|------|----------|
| 角色属性 | BaseStats, Experience | 序列化 Component |
| 装备/背包 | Equipment, Inventory | 序列化物品ID+实例数据 |
| 技能状态 | SkillSet | 已学技能列表+等级 |
| 任务进度 | QuestTracker | 任务状态表 |
| 世界状态 | WorldState | 已开宝箱、已触发事件、NPC状态 |
| 储物柜 | Storage containers | 序列化 slots |
| 玩家位置 | Transform | 场景ID + 坐标 |
| 游戏设置 | Settings | 独立文件，不随存档 |
...
# 各系统数据
@export var character_data: Dictionary = {}    # 角色属性/等级/经验
@export var inventory_data: Dictionary = {}    # 背包/装备
@export var skill_data: Dictionary = {}        # 技能
@export var quest_data: Dictionary = {}        # 任务
@export var world_state: Dictionary = {}       # 世界状态
@export var storage_data: Dictionary = {}      # 储物柜
...
```
存档时：
    ├── EcsWorld 遍历所有标记为 "saveable" 的 Entity
    │   └── 序列化其所有 Component 数据
    ├── DlcManager 记录当前已加载的 DLC 列表
    │   └── DLC 扩展的 Component 数据也一并序列化
    └── 写入 .tres 文件

读档时：
    ├── 先加载 DLC（确保 Component 类型已注册）
    ├── 创建/恢复 Entity
    │   └── 反序列化 Component 数据
    └── 各 System 自动开始处理恢复后的数据
...
提供全局的发布/订阅机制，让系统间通过事件名解耦通信，避免直接引用。
...
| `skill_learned` | 学习技能 | { skill_id, level } |
| `skill_used` | 使用技能 | { skill_id, target_ids } |
| `quest_started` | 任务开始 | { quest_id } |
| `quest_completed` | 任务完成 | { quest_id, rewards } |
| `scene_entered` | 进入场景 | { scene_id } |
| `dialogue_started` | 对话开始 | { npc_id, dialogue_id } |
...
```
ECS System 内部处理完逻辑后 → EventBus.emit_event() 通知外部
外部系统（UI、音效、成就）→ EventBus.subscribe() 监听并响应

示例：
LevelUpSystem 升级后 → emit("player_leveled_up", { new_level: 10 })
    ├── UI 系统监听 → 显示升级特效
    ├── 音效系统监听 → 播放升级音效
    ├── 成就系统监听 → 检查"达到10级"成就
    └── 任务系统监听 → 检查"升到10级"任务目标
```
...
| 3 | `AIDOC/global-info/knowledge/engines/godot/` 知识文件 | 知识库 | Req 5 |
| 4 | `AIDOC/global-info/knowledge/patterns/` 模式文件 | 知识库 | Req 5 |
| 5 | `AIDOC/global-info/knowledge/templates/` 模板文件 | 知识库 | Req 5 |
