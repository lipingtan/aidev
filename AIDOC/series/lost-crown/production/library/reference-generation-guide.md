...
参考图提示词（`output/assets/`）与视频片段提示词（`output/kling/`）的风格职责严格分离：

| | 参考图提示词（assets/） | 视频片段提示词（kling/） |
|--|------------------------|------------------------|
| 目的 | 让 KLING 提取物理特征，创建元素 | 渲染最终画面 |
| style-bible 风格词 | **❌ 禁止使用** | **✅ 每镜头开头必须包含** |
| 允许的质量词 | `photorealistic, high detail` | 全部 |
| 禁止的词 | `cinematic, dark fantasy, dramatic lighting, epic, cinematic quality` 等渲染风格词 | 无限制 |
| 光线描述 | 仅用 `soft even lighting` / `even lighting`（中性均匀光） | 按场景需要自由描述 |

**原因**：参考图越"干净中性"，KLING 在视频生成时越容易适配不同光线和氛围。风格渲染由视频提示词中的 style-bible 锚定词统一控制。

**素体与服装分离原则**：
- 素体参考图（`{角色}_body`）：包含面部/体型/肤色，穿简单内衣，**不含具体服装**
- 服装参考图（`{角色}_w{N}`）：展示完整服装+配饰，使用**无脸模特**（faceless mannequin），**不含角色面部特征**
- 使用时通过 `@素体 + @服装` 组合，KLING 从素体提取面部/体型，从服装提取穿着
