...
1. **后端技术栈**：现有后端为 Go 语言实现（go-admin 框架），非 Java。dev-web-admin 原配套的 Java 后端源码位于 `projects/demo/platform_admin/bak/spmp-backend/`，仅作为理解 dev-web-admin 前端逻辑的参考，不用于生产。
