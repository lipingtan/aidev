...
## V2-CR8 缓存事件驱动 + OAuth2 预留 + 扩展能力收尾 E2E 测试报告
...
| 测试环境 | http://localhost:8000 |
| 测试账号 | admin / admin123 |
| 测试框架 | Playwright（API 模式） |
| 测试脚本 | `e2e/tests/v2-cr8-cache-event-oauth.api.spec.ts` |
| 关联文档 | `v2-cr8-cache-event-oauth/test_cases.md` |
...
| FR | 描述 | 验证结果 |
|----|------|---------|
| FR-6 | OAuth2 骨架返回 501 | ✅ TC-A08 |
| FR-7 | LDAP 骨架返回 501 | ✅ TC-A09 |
| FR-1/4 | 权限检查缓存降级透明 | ✅ TC-A01/A02 |
| FR-4/5 | 操作日志 risk_level 字段 + 过滤 | ✅ TC-A04~A07 |
| FR-8 | ext_fields 向后兼容 | ✅ TC-A11~A14 |
| FR-9 | admin_custom_field DDL | ✅ TC-A15 |

### 修复的 Bug

| Bug | 描述 | 状态 |
|-----|------|------|
| BUG-001 | mapHTTPStatus 缺少 50xxx 映射（OAuth2/LDAP 返回 500 → 修复为 501） | ✅ 已修复 |
