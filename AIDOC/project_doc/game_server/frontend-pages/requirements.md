# 需求：game_server 前端页面补全

## 背景

`projects/demo/game_server` 的 backend 已完整实现 game 模块六类 API（游戏、DLC、玩家、订单、支付配置、H5页面）。frontend `views/game/` 下目前只有 `index.vue`（游戏管理），其余五个模块页面缺失，需补全。
