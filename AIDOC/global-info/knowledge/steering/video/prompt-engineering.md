...
**素体+服装引用格式（强制）**：
- 必须使用 `@素体 wearing @服装` 格式
- 禁止使用 `@素体 in @服装`（语义模糊，引擎可能误解为"在服装里面"）

**多人场景防呆板规则**：

| 问题 | 原因 | 解决方案 |
|------|------|----------|
| 多人并排站立 | 描述"N人站在一起"引擎默认排成一排 | 用动作/姿态区分每人位置，或改为纯光效/剪影暗示 |
| 人物面部怪异 | 无元素绑定时引擎随机生成面孔 | 无绑定的人物必须用纯剪影/背影/远景模糊处理 |
| 群体构图呆板 | "X人做同一件事"导致复制粘贴感 | 每人给不同动作/姿态/朝向 |

**核心原则**：
1. **无元素绑定 = 不画脸**：没有 @ID 的人物只能以剪影、背影、极远景出现，负面提示词加 `human figures, faces, people`
2. **多人 ≤ 2 人/镜头**：KLING 处理 3+ 人物时质量急剧下降，超过 2 人改用剪影或分镜头表现
3. **禁止对称并排**：多人同框时必须有前后景深差、不同姿态、不同朝向
4. **暗示优于直接展示**：用光效、影子、声音暗示人物存在，比直接画出来效果更好
